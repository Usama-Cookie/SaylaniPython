...
import math
position = [0,0]
directions = input('Enter direction to move. (Type exit to exit): ')
while directions != 'exit':

    if not directions:
        break
    movement = directions.split(" ")
    ...
    if direction=="UP":
        ...
        pass
    directions = input('Enter direction to move. (Type exit to exit): ')
x = int(round(math.sqrt(position[1]**2+position[0]**2)))
print('The distance from current position is: ',x)